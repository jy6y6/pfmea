...
def init_session():
    ...
    if "selected_image_idx" not in st.session_state:
        st.session_state.selected_image_idx = None  # 用于交换选中的图片索引
...
def excel_image_tool():
    ...
    if uploaded_files:
        ...
        st.session_state.selected_image_idx = None  # 重置选中

    # 实时预览（缩略图网格，支持点击交换）
    if st.session_state.uploaded_images and total_cells > 0:
        st.subheader("📋 实时预览（点击图片交换位置）")
        st.markdown("**提示**：点击任意图片选中，再点击另一张图片即可交换位置。")
        # 计算预览网格行列
        preview_rows = rows
        preview_cols = cols
        # 创建网格布局
            ...
            for c in range(preview_cols):
                ...
                if idx < len(st.session_state.uploaded_images):
                    ...
                    with cols_layout[c]:
                        # 用HTML模拟点击区域
                        st.markdown(f"""
                        <div class='preview-cell {selected_class}' id='img_{img_idx}' 
                             onclick='parent.postMessage({{type: "streamlit:setComponentValue", value: {img_idx}}}, "*")'
                             style='cursor:pointer;'>
                            <img src='data:image/jpeg;base64,{base64.b64encode(img_bytes).decode()}'/>
                            <div class='img-name'>{img_name[:12]}</div>
                        </div>
                        """, unsafe_allow_html=True)
                        # 由于 Streamlit 组件交互限制，这里用 st.button 模拟点击，但更可靠的是使用 st.button 加回调
                        # 改用 st.button 来实现点击交换（更可靠）
                        if st.button(f"📷 {img_name[:10]}", key=f"cell_{r}_{c}_{img_idx}", use_container_width=True):
                            if st.session_state.selected_image_idx is None:
                                st.session_state.selected_image_idx = img_idx
                                st.toast(f"已选中图片: {img_name}", icon="🖼️")
                            else:
                                # 交换
                                first = st.session_state.selected_image_idx
                                second = img_idx
                                # 获取两个图片在 order 中的位置
                                order = st.session_state.image_order
                                pos1 = order.index(first)
                                pos2 = order.index(second)
                                order[pos1], order[pos2] = order[pos2], order[pos1]
                                st.session_state.image_order = order
                                st.session_state.selected_image_idx = None
                                st.success(f"已交换图片 {st.session_state.uploaded_images[first][0][:15]} ↔ {st.session_state.uploaded_images[second][0][:15]}")
                                st.rerun()
                else:
                    with cols_layout[c]:
                        st.markdown("<div class='preview-cell' style='background:#f8f8f8;'>空</div>", unsafe_allow_html=True)
